...
def reliability_diagram(
    prob, Y, marker="--", label="", alpha=1, linewidth=1, ax_reliability=None, clip=True
):
    """
    ...
    centroids = [
        np.mean(
            np.append(
                prob[np.where(np.logical_and(prob >= bins[i], prob < bins[i + 1]))],
                bins[i] + 0.05,
            )
        )
        for i in range(len(hist_tot[1]) - 1)
    ]
    ...
    ax_reliability.plot(centroids, proportion, marker, linewidth=linewidth, label=label)


def plot_reliability_diagram(
    scores_set, labels, legend_set, original_first=False, alpha=1, **kwargs
):
    ...
    if original_first:
        ...
        empirical_p = np.insert(empirical_p, np.arange(len(empirical_p)), empirical_p)
...
def plot_reliability_map(
    scores_set, prob, legend_set, original_first=False, alpha=1, **kwargs
):
    ...
    for (scores, legend) in zip(scores_set, legend_set):
        if skip and original_first:
            ...
            if legend == "uncalib":
                ax.plot([np.nan], [np.nan], "-", linewidth=n_lines, **kwargs)
            else:
                ax.plot(prob[2], scores, "-", label=legend, linewidth=n_lines, **kwargs)
            n_lines -= 1
    if original_first:
        ax.plot(
            prob[0], prob[1], "kx", label=legend_set[0], markersize=9, markeredgewidth=1
        )
    ax.legend(loc="upper left")
    ax.grid(True)
...
if __name__ == "__main__":
    ...
    y = np.append(np.random.binomial(1, 0.1, 40), np.random.binomial(1, 0.3, 40))
    ...
    s_exp = sigmoid(s)
    fig_map = plot_niculescu_mizil_map([probas], [s_exp, y, l], ["beta"], alpha=1)
