...
def table_to_latex(datasets, methods, table, max_is_better=True, caption=""):
    ...
    means = table.to_numpy()[:, : len(methods)]
    avg_ranks = np.zeros(len(methods))
    if max_is_better:
        means *= 100.0
    stds = table.to_numpy()[:, len(methods) :]
    if max_is_better:
        stds *= 100.0
    # print('\\begin{table}[!t]')
    # print('\\centering')
    str_columns = "l"
    str_header = "dataset"
    methods = [(x or "None") for x in methods]  # to make None -> "None"
    methods.sort()
...
def to_latex(
    ...
    means = table.to_numpy()[:, :n_columns] * scale
    avg_ranks = np.zeros(n_columns)
    stds = table.to_numpy()[:, n_columns:] * scale
    ...
    for i, name in enumerate(row_names):
        ...
        for j in np.arange(len(v)):
            ...
            if idx == 1:
                ...
                #                 precision))
                str_row_means += (
                    " & $\\mathbf{{{0:.{2}f}\\pm{1:.{2}f}_{{{3}}}}}$".format(
